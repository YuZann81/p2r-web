...
export async function fetchGameLeaderboard(slug: string): Promise<LeaderboardEntry[]> {
  try {
    const payload = await apiGet<RawLeaderboardEntry[]>(
      `/leaderboards/games/${encodeURIComponent(slug)}`,
    );
    const rawList = Array.isArray(payload.data) ? payload.data : [];
    return rawList.map((entry) => {
      const playerName = entry.player_name || entry.playerName || "Anonymous";
      const gameName = entry.game?.name || entry.gameName || entry.game_name || "Arcade Game";
      const gameSlug = entry.game?.slug || slug;
      const score =
    ...
    console.warn(
      `[p2r-api] Game leaderboard (${slug}) unavailable/empty:`,
...
export async function fetchKaryaLeaderboard(): Promise<KaryaLeaderboardEntry[]> {
  try {
    ...
    return rawList.map((entry) => {
      ...
      const votesCount =
        typeof entry.votes_count === "number"
          ? entry.votes_count
          : typeof entry.votesCount === "number"
            ? entry.votesCount
            : 0;
    ...
    console.warn(
      "[p2r-api] Karya leaderboard unavailable/empty:",
      error instanceof Error ? error.message : error,
    );
    return [];
  }
